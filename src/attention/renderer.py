...
class Renderer:
    ...
    def _draw_shape_geometry(self, shape: AttentionShape, pen: QPen, brush=None):
        ...
        if isinstance(shape, RectangleShape):
            ...
            self.painter.drawEllipse(
                QPoint(int(center_x), int(center_y)), int(radius_x), int(radius_y)
            )
    ...
    def _draw_shape(self, shape: AttentionShape):
        ...
        if isinstance(shape, LabelShape):
            ...
            brush_color = self._get_color_with_opacity(
                QColor(128, 0, 128), shape.opacity, 0.4
            )  # 40% alpha base
            brush = brush_color
        else:
    ...
    def _draw_text(self, shape: AttentionShape):
        if isinstance(shape, LabelShape):
            ...
            self.painter.drawText(
                int(shape.x), int(shape.y - 2), f"{shape.text} ({shape.confidence})"
            )
